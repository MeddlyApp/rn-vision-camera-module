...
export default function RenderCamera(props: Props) {
  ...
  const isPortrait = orientation === 'PORTRAIT'.toLowerCase();
  const isLandscapeRight = orientation === 'LANDSCAPE-RIGHT'.toLowerCase();
  const ratioPortrait = 1080 / 1920;
  const ratioLandscape = 1920 / 1080;
  const ratio = isPortrait ? ratioPortrait : ratioLandscape;
  ...
  //  {photoResolution: 'max'},
  //);
  //console.log({devicesLength: devices});
  //const device: CameraDevice | undefined = useMemo(
  //  devices.find((x: any) => (x.position === frontCamera ? 'front' : 'back')),
  //  [devices, frontCamera],
  //);
  ...
  const format = useMemo(() => {
    ...
    const filtered720pFormats = device?.formats.filter(
      (format: CameraDeviceFormat) => {
        ...
        const videoIsHeight = format.videoHeight === height;
        const videoIsWidth = format.videoWidth === width;
        const videoIs720p = videoIsHeight && videoIsWidth;

        const photoIsHeight = format.photoHeight === height;
        const photoIsWidth = format.photoWidth === width;
        const photoIs720p = photoIsHeight && photoIsWidth;

        return videoIs720p && photoIs720p;
      },
    );
